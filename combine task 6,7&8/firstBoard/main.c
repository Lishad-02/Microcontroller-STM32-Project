...
uint8_t txSignal = 0x01;  // Data to send
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  // Read DHT11 sensor
	    ...
	    // ✅ Send signal to second board
	    if (Distance < 10)
	    {
	      HAL_UART_Transmit(&huart2, &txSignal, 1, HAL_MAX_DELAY);
	    }



	    // Display all data on OLED
	    SSD1306_Clear();
	    ...
	    sprintf(strCopy, "Dist: %d cm", Distance);
	    SSD1306_GotoXY(0, 35);
	    SSD1306_Puts(strCopy, &Font_7x10, 1);
	    ...
	    HAL_Delay(2000); // 2 seconds delay between updates
