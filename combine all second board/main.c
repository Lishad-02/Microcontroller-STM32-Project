...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	    ...
	    // IR Sensor Control for Servo
	    if (HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_0) == GPIO_PIN_RESET) // IR active low
	    {
	      int x;
	      for (x = 250; x < 1250; x++)
	      {
	    ...
	    {
	      __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_2, 750); // Neutral position
	    }
    /* USER CODE BEGIN 3 */
  }
  /* USER CODE END 3 */
